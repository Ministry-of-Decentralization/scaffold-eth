...
describe("PixelBoard", function() {
  it("Should create default walls on deploy", async function() {
    ...
    const wallRow = await alleybox.getWallColors(0)
  ...
  it("Should support getWallRow to be queried", async function() {
    ...
    await alleybox.deployed();
    const wallRow = await alleybox.getWallRow(0, 0)
  ...
  it("Should be able to update bricks", async function() {
    ...
    const wallRows = await alleybox.getWallRow(0,0)
    ...
    await alleybox.updateBricks([brick2.w], [brick2.x], [brick2.y], [newColor2], ['1'], {value: '1'})
    const updatedBrick2 = await alleybox.getBrick(brick2.w, brick2.x, brick2.y)

    expect(updatedBrick2.rgb).to.equal(newColor2);

    const wallRows2 = await alleybox.getWallRow(1,brick2.y)
    console.log(wallRows2)
    
    expect(wallRows2[brick2.x]).to.equal(newColor2)
